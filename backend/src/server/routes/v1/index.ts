...
import { registerSecretSharingPublicRouter, registerSecretSharingRouter } from "./secret-sharing-router";
...
export const registerV1Routes = async (server: FastifyZodProvider) => {
  ...
  await server.register(
    async (projectRouter) => {
      ...
      await projectRouter.register(registerSecretSharingRouter);
    },
  ...
  await server.register(registerSecretSharingPublicRouter, { prefix: "/secret-sharing" });
};
